...
Get-WinEvent -Computername $env:computername -FilterHashTable @{LogName='Security';ID='4625';StartTime=(Get-Date).AddDays(
-7);} | Get-WinEventData | fl | out-file "$($env:computername).$($date).successful-logins-1week.txt"

Get-WinEvent -Computername $env:computername -FilterHashTable @{LogName='Security';ID='4624';StartTime=(Get-Date).AddDays(
-7);} | Get-WinEventData | fl | out-file "$($env:computername).$($date).successful-logins-1week.txt"
