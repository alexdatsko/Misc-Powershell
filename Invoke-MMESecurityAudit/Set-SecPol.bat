...
cls
::
:: MME Local Security Policy - Enable logging (will be overruled by enabled Group Policy!)
:: Alex Datsko - alexd@mmeconsulting.com
:: v0.11 - Changed powershell logging location to c:\windows\logs\Powershell and protected this folder
::
:: Capture the local security policy first, BEFORE changes.
Auditpol /get /category:* > AuditPol_BEFORE_%computername%.txt
::
:: Make the folders..

mkdir C:\Temp
mkdir %windir%\Logs\Powershell
icacls %windir%\Logs\Powershell /reset
icacls %windir%\Logs\Powershell /inheritance:r
icacls %windir%\Logs\Powershell /inheritance:d
icacls %windir%\Logs\Powershell /grant Administrators:F 
icacls %windir%\Logs\Powershell /grant "NT SERVICE\TrustedInstaller":F
icacls %windir%\Logs\Powershell /grant "NT AUTHORITY\SYSTEM":F
icacls %windir%\Logs\Powershell /remove "BUILTIN\Users"
icacls %windir%\Logs\Powershell /remove "Users"
icacls %windir%\Logs\Powershell


:: -Now should look like: 
:: C:\Windows\Logs>icacls %windir%\Logs\Powershell
:: C:\Windows\Logs\Powershell NT AUTHORITY\SYSTEM:(F)
...
::
wevtutil sl Security /ms:32000000100
::
...
::
:: Mkdir moved to above
::
reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\PowerShell\Transcription" /v OutputDirectory /t REG_SZ /d "%windir%\Logs\Powershell" /f
...
::
:: WARNING:  This next item is a VERY noisy items and requires the Windows Firewall to be in at least an ALLOW ALLOW configuration in Group Policy
::
