...
Get-WinEvent -Computername $computer -Credential $cred -FilterHashTable @{LogName='Security';ID='4625';StartTime=(Get-Date).AddDays(
-7);} | Get-WinEventData | Select TimeCreated, EventDataTargetUserName | export-Csv $($env:computername)-$date-successful-logins-1week.csv

Get-WinEvent -Computername $computer -Credential $cred -FilterHashTable @{LogName='Security';ID='4624';StartTime=(Get-Date).AddDays(
-7);} | Get-WinEventData | Select TimeCreated, EventDataTargetUserName | export-Csv $($env:computername)-$date-successful-logins-1week.csv
