...
.MODIFIED 2024-01-31 Alex Datsko
...
[CmdletBinding()]
    ...
    [string]$LogFileName = "BitLockerInfo_$($env:userdnsdomain)_$(Get-Date -f 'yyyy-MM-dd').csv"
    )
...
if ($OU -ne "") {
    ...
    $Computers = Get-ADComputer -Filter 'ObjectClass -eq "computer"' -Property * | Where-Object { $_.LastLogonDate -ge $threeMonthsAgo -and $_.Enabled -eq $true } 
}

write-host "Report File Path:" $LogFile

#Write report header
$strToReport = "ComputerName" + $strDelimiter + "OperatingSystem" + $strDelimiter + "Date" + $strDelimiter + "Time" + $strDelimiter + "RecoveryID" + $strDelimiter + "RecoveryPassword"
...
foreach ($Computer in $Computers) {
    ...
    $strComputerTime = ""
    $strComputerGMT = ""
    $strComputerPasswordID = ""
    $strComputerRecoveryPassword = ""
    ...
    $strToReport = $Computer.Name + $strDelimiter + $Computer.OperatingSystem + $strDelimiter + $strComputerDate + $strDelimiter + $strComputerTime + $strDelimiter + $strComputerPasswordID + $strDelimiter + $strComputerRecoveryPassword
