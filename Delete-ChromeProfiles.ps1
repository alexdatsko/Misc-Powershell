    ...
    if ($Folders) {
      Foreach ($Folder in $Folders) {
        ...
        if (Test-Path "$($Folder)\Service Worker\CacheStorage") {
          ...
          $FilesToDelete = (GCI "$($Folder)\Service Worker\CacheStorage\" -Recurse -ErrorAction SilentlyContinue).FullName 
          #$FilesToDelete | tee -append $LogFile
          "Removing $($Folder)\Service Worker\CacheStorage\*.*" | tee -append $LogFile
