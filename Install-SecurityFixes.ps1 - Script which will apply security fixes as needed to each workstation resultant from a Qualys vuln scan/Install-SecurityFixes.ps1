...
#$tmp = "$($env:SystemRoot)\Temp\SecAud"      # Temporary folder to save downloaded files to
...
$QIDsVirtualBox = 372509,372512,372542,373154,373553,374881,375481,375736,375967,376736,376548,376255
...
foreach ($QID in $QIDs) {
    ...
    {
      376023 { 
        $yesno = Read-Host "[Y] QID376023- Remove SupportAssist ? [n] " ; if ($yesno.ToUpper()[0] -eq 'Y') {
      ...
      105228 { 
        $yesno = Read-Host "[Y] QID105228 - disable guest account and rename ? [n] " ; if ($yesno.ToUpper()[0] -eq 'Y') {
            Rename-LocalUser -Name "Guest" -NewName "GuestAcctNew" | Disable-LocalUser
        }
      }
      { $QIDSpectreMeltdown -contains $_ } {
        $yesno = Read-Host "[Y] QID91537,91462,91426 - spectre/meltdown ? [n] " ; if ($yesno.ToUpper()[0] -eq 'Y') {
      ...
      91738 {
        $yesno = Read-Host "[Y] QID 91738(?) - fix ipv4 source routing bug/ipv6 global reassemblylimit? [n] " ; if ($yesno.ToUpper()[0] -eq 'Y') { 
            netsh int ipv4 set global sourceroutingbehavior=drop
            Netsh int ipv6 set global reassemblylimit=0
        }
      }
      375589 {  
        $yesno = Read-Host "[Y] QID375589 - Delete Dell DbUtil_2_3.sys ? [n] " ; if ($yesno.ToUpper()[0] -eq 'Y') {
            cmd /c 'del c:\users\dbutil_2_3*.sys /s /f /q'
        }
      }
      100413 {
        $yesno = Read-Host "[Y] QID100413 - CVE-2017-8529 - IE Feature_Enable_Print_Info_Disclosure fix ? [n] " ; if ($yesno.ToUpper()[0] -eq 'Y') {
      ...
      { 105170,105171 -contains $_ } { 
        $yesno = Read-Host "[Y] QID105170, QID105171 - Windows Explorer Autoplay not Disabled ? [n] " ; if ($yesno.ToUpper()[0] -eq 'Y') {
      ...
      90044 {
        $yesno = Read-Host "[Y] QID90044 - Allowed SMB Null session ? [n] " ; if ($yesno.ToUpper()[0] -eq 'Y') {
      ...
      90007 {
        $yesno = Read-Host "[Y] QID90007 - Enabled Cached Logon Credential ? [n] " ; if ($yesno.ToUpper()[0] -eq 'Y') {
      ...
      90043 {
        $yesno = Read-Host "[Y] QID90043 - SMB Signing Disabled / Not required ? [n] " ; if ($yesno.ToUpper()[0] -eq 'Y') {
      ...
      91805 {
        $yesno = Read-Host "[Y] QID91805 - Remove Windows10 UpdateAssistant? [n] " ; if ($yesno.ToUpper()[0] -eq 'Y') {
      ...
      372348 {
        $yesno = Read-Host "[Y] QID372348 - Intel Chipset INF util ? [n] " ; if ($yesno.ToUpper()[0] -eq 'Y') {
      ...
      372300 {
        $yesno = Read-Host "[Y] QID372300 - Intel RST ? [n] " ; if ($yesno.ToUpper()[0] -eq 'Y') {
      ...
      { $QIDsIntelGraphicsDriver  -contains $_ } {
        $yesno = Read-Host "[Y] QID $_ Install newest Intel Graphics Driver? [n] "
        if ($yesno.ToUpper()[0] -eq 'Y') { 
      ...
      { $QIDsChrome -contains $_ } {
        $yesno = Read-Host "[Y] Install newest Google Chrome? [n] "
        if ($yesno.ToUpper()[0] -eq 'Y') { 
      ...
      { $QIDsFirefox -contains $_ } {
        $yesno = Read-Host "[Y] Install newest Firefox? [n] "
        if ($yesno.ToUpper()[0] -eq 'Y') { 
      ...
      { $QIDsZoom -contains $_ } {
        $yesno = Read-Host "[Y] Install newest Zoom Client? [n] "
        if ($yesno.ToUpper()[0] -eq 'Y') { 
      ...
      { $QIDsTeamViewer15 -contains $_ } {
        $yesno = Read-Host "[Y] Install newest Teamviewer 15? [n] "
        if ($yesno.ToUpper()[0] -eq 'Y') { 
      ...
      { $QIDsDropbox -contains $_ } {
        $yesno = Read-Host "[Y] Install newest Dropbox? [n] "
        if ($yesno.ToUpper()[0] -eq 'Y') { 
      ...
      { $QIDsOracleJava -contains $_ } {
        $yesno = Read-Host "[Y] Install newest Oracle Java 18? [n] "
        if ($yesno.ToUpper()[0] -eq 'Y') { 
      ...
      { $QIDsOracleJavaSE -contains $_ } {
        $yesno = Read-Host "[Y] Install newest Oracle Java SE 18? [n] "
        if ($yesno.ToUpper()[0] -eq 'Y') { 
      ...
      { $QIDsAdoptOpenJDK -contains $_ } {
        $yesno = Read-Host "[Y] Install newest Adopt Java JDK? [n] "
        if ($yesno.ToUpper()[0] -eq 'Y') { 
      ...
      { $QIDsVirtualBox -contains $_ } {
        $yesno = Read-Host "[Y] Install newest VirtualBox 6.1.36? [n] "
        if ($yesno.ToUpper()[0] -eq 'Y') { 
      ...
      { $QIDsAdobeReader -contains $_ } {
        $yesno = Read-Host "[Y] Install newest Adobe Reader DC? [n] "
        if ($yesno.ToUpper()[0] -eq 'Y') { 
      ...
      { $QIDNVIDIAPrivEsc -contains $_ } {
        ...
        if ($yesno.ToUpper()[0] -eq 'Y') { 
            ...
            if ($NvidiacardFound){
              ...
              $yesno = Read-Host "[Y] Remove NVIDIA PrivEsc exe c:\windows\system32\nvvsvc.exe ? [n] "
              if ($yesno.ToUpper()[0] -eq 'Y') { 
